	...
	var handleRecords = function () {
        ...
        "stateSave": true, // save datatable state(pagination, sort, etc) in cookie.
				...
				"columns": [
					...
					{"data": 3, className: 'text-center',"name":'varPhoneNo'},
				...
				"ajax": {
					"url": window.site_url+"/powerpanel/service-inquiry/get_list", // ajax source
				},
				"order": [
					[7, "desc"]
		...
		$(document).on('keyup', '#searchfilter', function (e) {
			...
			if (action.length>=2) {				
				$.cookie('ServiceinquiryLeadsearch',action);				
				grid.setAjaxParam("customActionType", "group_action");
				grid.setAjaxParam("searchValue", action);
				grid.setAjaxParam("id", grid.getSelectedRows());
				grid.getDataTable().ajax.reload();
			} else if(action.length<1) {
				$.removeCookie('ServiceinquiryLeadsearch');
				grid.setAjaxParam("customActionType", "group_action");
				grid.setAjaxParam("searchValue", action);
				grid.setAjaxParam("id", grid.getSelectedRows());
				grid.getDataTable().ajax.reload();
		...
		$(document).on('change', '#servicefilter', function (e) {
            e.preventDefault();
            var action = $('#servicefilter').val();
            if (action != "") {
                grid.setAjaxParam("customActionType", "group_action");
                grid.setAjaxParam("servicefilter", action);
                grid.setAjaxParam("id", grid.getSelectedRows());
                grid.getDataTable().ajax.reload();
            } else {
                grid.setAjaxParam("customActionType", "group_action");
		...
		$(document).on('click', '#refresh', function (e) {
            ...
            grid.setAjaxParam("servicefilter", '');
            grid.getDataTable().ajax.reload();
        });
					...
					if (exportRadioVal == 'selected_records') {
						if ($('#ExportRecord').click) {
							if ($('input[name="delete[]"]:checked').val()) {
								...
								var ajaxurl = window.site_url+"/powerpanel/service-inquiry/ExportRecord"+ip;
						...
						var ajaxurl = window.site_url+"/powerpanel/service-inquiry/ExportRecord";
						window.location = ajaxurl;
					}
				}
			}
...
$(window).on('load',function(){
	if($.cookie('ServiceinquiryLeadsearch')){
		$('#searchfilter').val($.cookie('ServiceinquiryLeadsearch'));
