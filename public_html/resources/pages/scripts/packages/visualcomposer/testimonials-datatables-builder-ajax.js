    ...
    return {
        ...
        moduleSectionsTestimonial: function (caption, config, configTxt, recids, recTitle, edit, template, layoutType, extra_class) {
            ...
            if (edit == 'N') {
                ...
                var testimonialCustomized = [];
                var testimonialDescription = [];
                ...
                $('.section-item[data-editor=' + edit + '] li').each(function (key, val) {
                    ...
                    if (typeof Icustomized != 'undefined') {
                        testimonialCustomized.push(Icustomized);
                    }

                    
                    var Idescription = $(this).data('description');
                    if (typeof Idescription != 'undefined') {
                        testimonialDescription.push(Idescription.toString());
                    }
                ...
                $.each(recids, function (index, value) {
                    ...
                    testimonialCustomized.push(false);
                    testimonialDescription.push('');
                });
                ...
                $.each(testimonialIds, function (index, value) {
                    if (value != '') {
                        section += '<li data-customized="' + testimonialCustomized[index] + '"  data-description="' + testimonialDescription[index] + '" data-id="' + value + '" id="' + value + '-item-' + edit + '">' + testimonialTitles[index]  + '<a href="javascript:;" class="close-icon" title="Delete"><i class="fa fa-times" aria-hidden="true"></i></a></li>';
                    }
                });
                section += '</ul>';
...
var TestimonialDataTable = function () {
    ...
    return {
        ...
        init: function (from, to) {
            ...
            jQuery.ajax({
                ...
                data: {
                    ...
                    columns: sort[0],
                    order: sort[1],
                    catValue: $('#sectionTestimonialModule #category-id').val(),
                    status: '',
        ...
        //     var ajaxUrl = site_url + '/powerpanel/testimonial-category/get_builder_list';
        //     jQuery.ajax({
        //         type: "POST",
        //         url: ajaxUrl,
        //         dataType: 'HTML',
        //         async: false,
        //         success: function (result) {
        //             $('#sectionTestimonialModule #category-id').html(result);
        //         }
        //     });
...
$('#sectionTestimonialModule').on('shown.bs.modal', function () {
    ...
    // TestimonialDataTable.getCategory();
    $('#frmSectionTestimonialModule #category-id option:first').prop('selected', true);
...
$(document).on('change', '#sectionTestimonialModule #category-id', function () {
    range = 10;
    start = 0;
    end = range;
    $('#sectionTestimonialModule #record-table').html('');
