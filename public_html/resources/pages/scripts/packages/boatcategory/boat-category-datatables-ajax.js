...
var TableDatatablesAjax = function() {
		...
		var handleRecords = function() {
			grid = new Datatable(); //BoatCategorySearch
			if(!tableState){
			  $.removeCookie('BoatCategorySearch');
	      $("#datatable_ajax").DataTable().state.clear();
	      $("#datatable_ajax").DataTable().destroy();
	    }
	    if (tableState) {
	    	  if($.cookie('BoatCategorySearch')) {
				      var BoatCategorySearch = $.cookie('BoatCategorySearch');
                $('#searchfilter').val(BoatCategorySearch);
                grid.setAjaxParam("searchValue", BoatCategorySearch);
				...
				grid.init({
						...
						dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
								...
								"ajax": {
										"url": window.site_url + "/powerpanel/boat-category/get_list", // ajax source
								},
				...
				$(document).on('change', '#statusfilter', function(e) {
						...
						if (action != " ") {
								//$.cookie('BoatCategoryStatus',action);
						} else {
								//$.removeCookie('BoatCategoryStatus');
						}
						if (action != "") {
				...
				$(document).on('keyup', '#searchfilter', function(e) {
						...
						if (action.length >= 2) {
								$.cookie('BoatCategorySearch',action);				
								grid.setAjaxParam("customActionType", "group_action");
...
function reorder(curOrder, excOrder) {
		var ajaxurl = site_url + '/powerpanel/boat-category/reorder';
		$.ajax({
				url: ajaxurl,
				data: {
						order: curOrder,
