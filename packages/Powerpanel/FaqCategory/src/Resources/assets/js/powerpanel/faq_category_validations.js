		...
		var handleNews = function () {
				$("#frmFaqCategory").validate({
						...
						invalidHandler: function (event, validator) { //display error alert on form submit  
								...
								$('.alert-danger', $('#frmFaqCategory')).show();
						},
				...
				$('#frmFaqCategory input').keypress(function (e) {
						if (e.which == 13) {
								if ($('#frmFaqCategory').validate().form()) {
										$('#frmFaqCategory').submit(); //form validation success, call ajax form submit
								}
								return false;
						}
				});
