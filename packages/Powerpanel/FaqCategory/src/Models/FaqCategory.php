...
class FaqCategory extends Model {
    ...
    public static function getRecordDataByAliasID($aliasID) {
        ...
        if (empty($response)) {
            $response = Self::Select(
                            'id', 'varTitle', 'txtDescription', 'varMetaTitle', 'varMetaKeyword', 'varMetaDescription', 'chrPageActive', 'intSearchRank', 'varPassword', 'chrDraft', 'chrTrash', 'FavoriteID','UserID', 'created_at', 'updated_at'
                    )
                    ->deleted()
                    ->publish()
                    ->dateRange()
    ...
    public static function getChildrollbackGrid() {
        $id = $_REQUEST['id'];
        $response = false;
        $cmsPageFields = ['id', 'intAliasId', 'dtDateTime', 'varTitle', 'txtDescription', 'intDisplayOrder', 'varMetaTitle', 'varMetaDescription', 'intSearchRank', 'chrPublish', 'chrDelete', 'UserID', 'chrApproved', 'fkMainRecord', 'intApprovedBy', 'chrPageActive', 'varPassword', 'chrDraft', 'chrTrash', 'FavoriteID', 'created_at', 'updated_at'];
        $aliasFields = ['id', 'varAlias'];
    ...
    public static function getRecordList_tab1($filterArr = false) {
        $response = false;
        $cmsPageFields = ['id', 'intAliasId', 'varTitle', 'dtDateTime', 'txtDescription', 'intDisplayOrder', 'intSearchRank', 'LockUserID', 'chrLock', 'varMetaTitle', 'varMetaDescription', 'chrPublish', 'chrDelete', 'chrAddStar', 'chrPageActive', 'varPassword', 'chrDraft', 'chrTrash', 'FavoriteID', 'created_at', 'updated_at'];
        $aliasFields = ['id', 'varAlias'];
        $userid = auth()->user()->id;
        $MainIDs = Self::distinct()
                ->select("fkMainRecord")
