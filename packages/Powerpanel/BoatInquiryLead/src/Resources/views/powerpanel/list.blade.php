...
<div class="row">
	<div class="col-md-12">
		...
		<div class="title-dropdown_sec">
				...
				@include('powerpanel.partials.listbreadcrumbs',['ModuleName'=>'Manage Boat Inquiry Leads'])
				@endif
							...
							<div class="row">
								<div class="col-lg-6 col-md-12 col-xs-12">
									<div class="portlet-lf-title">
										<span class="select_input" style="display:inline-block;vertical-align:middle;">
											<select id="boatfilter" data-sort data-order class="bs-select select2" data-width="250">
												<option value="">--Select Interested In--</option>
												@foreach($boatsName as $boatsData)
												@php $permissionName = 'boats-list' @endphp
												@php
												$SelectedId = isset($_REQUEST['id']); 
												if($boatsData->id == $SelectedId){
												$selected = 'selected="selected"';
												}else{
												$selected = '';
												}
												@endphp 
											<option value="{{ $boatsData->id }}" {{ $selected }}>{{ $boatsData->varTitle }}</option>
											@endforeach
											</select>
											</span> 
								...
								<div class="col-lg-6 col-md-12 col-xs-12" style="display:none;">
									<div class="portlet-rh-title">
										...
										<a class="btn btn-green-drake pull-right btn-rh-search " style="margin-right:15px;" id="boatinquiryleadrange" href="javascript:;"><i class="fa fa-search"></i></a>
				...
				<table class="new_table_desing table table-striped table-bordered table-hover table-checkable dataTable" id="datatable_ajax">
					<thead>
						<tr role="row" class="heading">
							...
							<th width="12%" align="left">{{ trans('boatinquirylead::template.common.name') }}</th>							
							<th width="15%" align="left">{{ trans('boatinquirylead::template.common.email') }}</th>
							<th width="12%" align="center">{{ trans('boatinquirylead::template.common.phoneno') }}</th>
							<th width="12%" align="left">{{ trans('boatinquirylead::template.common.boatname') }}</th>
							<th width="8%" align="center">Comment</th>
							<th width="9%" align="center">{{ trans('boatinquirylead::template.common.ip') }}</th>
							<th width="15%" align="center">{{ trans('boatinquirylead::template.common.received_date_time') }}</th>
						</tr>
					</thead>
					<tbody></tbody>
				</table>				
				@can('boat-inquiry-delete')
				<a href="javascript:;" class="btn-sm btn btn-outline red right_bottom_btn deleteMass">{{ trans('boatinquirylead::template.common.delete') }}</a>
				@endcan
				<a href="#selectedRecords" class="btn-sm btn btn-green-drake right_bottom_btn ExportRecord" data-toggle="modal">{{ trans('boatinquirylead::template.boatinquiryleadModule.export') }}</a>				
			</div>
		</div>
		@else
		...
		<div class="modal-vertical">	
			<div class="modal-content">
					<div class="modal-header">
							...
							{{ trans('boatinquirylead::template.common.alert') }} 
					</div>
					<div class="modal-body text-center">{{ trans('boatinquirylead::template.boatinquiryleadModule.noExport') }} </div>
					<div class="modal-footer">
							<button type="button" class="btn btn-green-drake" data-dismiss="modal">{{ trans('boatinquirylead::template.common.ok') }}</button>
					</div>
			</div>
		</div>
	</div>
	...
	<div class="modal-dialog">
		<div class="modal-vertical">	
			<div class="modal-content">
					...
					<div class="modal-body text-center">{{ trans('boatinquirylead::template.boatinquiryleadModule.recordsExport') }}</div>
					<div class="modal-footer">
						<div align="center">
							<div class="md-radio-inline">
								<div class="md-radio">
									...
									<label for="selected_records">
										...
										<span class="box"></span> {{ trans('boatinquirylead::template.boatinquiryleadModule.selectedRecords') }}
									</label>
								</div>
								<div class="md-radio">
									...
									<label for="all_records">
										<span class="inc"></span>
										<span class="check"></span>
										<span class="box"></span>{{ trans('boatinquirylead::template.boatinquiryleadModule.allRecords') }} 
									</label>
								</div>
							</div>
						</div>
						<button type="button" class="btn btn-green-drake" id="ExportRecord" data-dismiss="modal">{{ trans('boatinquirylead::template.common.ok') }} </button>
					</div>
			</div>
			<!-- /.modal-content -->
		</div>
	...
	<div class="modal-dialog">
		<div class="modal-vertical">	
			<div class="modal-content">
				...
				<div class="modal-body text-center">{{ trans('boatinquirylead::template.boatinquiryleadModule.leastRecord') }} </div>
				<div class="modal-footer">
					<button type="button" class="btn btn-green-drake" data-dismiss="modal">{{ trans('boatinquirylead::template.common.ok') }} </button>
				</div>
			</div>
...
@section('scripts')
	<script type="text/javascript">
		window.site_url =  '{!! url("/") !!}';
		var DELETE_URL =  '{!! url("/powerpanel/boat-inquiry/DeleteRecord") !!}';
	</script>
	...
	<script src="{{ url('resources/pages/scripts/packages/boatinquirylead/boatinquirylead-datatables-ajax.js') }}" type="text/javascript"></script>
