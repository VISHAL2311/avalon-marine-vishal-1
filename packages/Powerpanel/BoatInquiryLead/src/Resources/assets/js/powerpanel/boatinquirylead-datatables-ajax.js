var TableDatatablesAjax = function () {
	...
	var handleRecords = function () {
        ...
        "stateSave": true, // save datatable state(pagination, sort, etc) in cookie.
				...
				"ajax": {
					"url": window.site_url+"/powerpanel/boat-inquiry/get_list", // ajax source
				},
				"order": [
					[7, "desc"]
		...
		$(document).on('keyup', '#searchfilter', function (e) {
			...
			if (action.length>=2) {				
				$.cookie('BoatinquiryLeadsearch',action);				
				grid.setAjaxParam("customActionType", "group_action");
				grid.setAjaxParam("searchValue", action);
				grid.setAjaxParam("id", grid.getSelectedRows());
				grid.getDataTable().ajax.reload();
			} else if(action.length<1) {
				$.removeCookie('BoatinquiryLeadsearch');
				grid.setAjaxParam("customActionType", "group_action");
				grid.setAjaxParam("searchValue", action);
				grid.setAjaxParam("id", grid.getSelectedRows());
				grid.getDataTable().ajax.reload();
		...
		$(document).on('change', '#boatfilter', function (e) {
            e.preventDefault();
            var action = $('#boatfilter').val();
            if (action != "") {
                grid.setAjaxParam("customActionType", "group_action");
                grid.setAjaxParam("boatfilter", action);
                grid.setAjaxParam("id", grid.getSelectedRows());
                grid.getDataTable().ajax.reload();
            } else {
                grid.setAjaxParam("customActionType", "group_action");
		...
		$(document).on('click', '#refresh', function (e) {
            $('#boatfilter').val('').trigger('change');
            grid.setAjaxParam("boatfilter", '');
            grid.getDataTable().ajax.reload();
        });
					...
					if (exportRadioVal == 'selected_records') {
						if ($('#ExportRecord').click) {
							if ($('input[name="delete[]"]:checked').val()) {
								...
								var ajaxurl = window.site_url+"/powerpanel/boat-inquiry/ExportRecord"+ip;
						...
						var ajaxurl = window.site_url+"/powerpanel/boat-inquiry/ExportRecord";
						window.location = ajaxurl;
					}
				}
			}
...
$(window).on('load',function(){
	if($.cookie('BoatinquiryLeadsearch')){
		$('#searchfilter').val($.cookie('BoatinquiryLeadsearch'));
