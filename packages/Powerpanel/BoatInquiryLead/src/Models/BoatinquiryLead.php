...
class BoatinquiryLead extends Model {
	/**
	 * The table associated with the model.
	 *
	 * @var string
	 */
	 protected $table = 'boatinquiry_lead';
	 protected $fillable = [
		'id',
		'fkIntBoatId',
		'varName',
...
/**
	...
	public static function getRecordList($filterArr=false){
		$response = false;
		$moduleFields=[ 'id','fkIntBoatId','varName','varEmail','varPhoneNo','txtUserMessage','varIpAddress','created_at','chrPublish'];
	...
	public static function getListForExport($selectedIds=false){
		$response = false;
		$moduleFields=[ 'varName','fkIntBoatId','varEmail','varPhoneNo','txtUserMessage','varIpAddress','created_at'];
	...
	function scopeFilter($query, $filterArr = false ,$retunTotalRecords = false) 
	{
				...
				if (!empty($filterArr['boatfilter']) && $filterArr['boatfilter'] != ' ') {
					$data = $query->where('fkIntBoatId', $filterArr['boatfilter']);
				}
