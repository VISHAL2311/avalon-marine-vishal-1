...
<div class="row">
	<div class="col-md-12">
		...
		<div class="title-dropdown_sec">
				...
				@include('powerpanel.partials.listbreadcrumbs',['ModuleName'=>'Manage Contact Leads'])
				@endif
				...
				<table class="new_table_desing table table-striped table-bordered table-hover table-checkable dataTable" id="datatable_ajax">
					<thead>
						<tr role="row" class="heading">
							...
							<th width="12%" align="left">{{ trans('contactuslead::template.common.name') }}</th>							
							<th width="15%" align="left">{{ trans('contactuslead::template.common.email') }}</th>
							<th width="12%" align="center">{{ trans('contactuslead::template.common.phoneno') }}</th>
							<th width="12%" align="left">{{ trans('Interested In') }}</th>
							<th width="8%" align="center">{{ trans('contactuslead::template.contactleadModule.message') }}</th>
							<th width="9%" align="center">{{ trans('contactuslead::template.common.ip') }}</th>
							<th width="15%" align="center">{{ trans('contactuslead::template.common.received_date_time') }}</th>
						</tr>
					</thead>
					<tbody></tbody>
				</table>				
				@can('contact-us-delete')
				<a href="javascript:;" class="btn-sm btn btn-outline red right_bottom_btn deleteMass">{{ trans('contactuslead::template.common.delete') }}</a>
				@endcan
				<a href="#selectedRecords" class="btn-sm btn btn-green-drake right_bottom_btn ExportRecord" data-toggle="modal">{{ trans('contactuslead::template.contactleadModule.export') }}</a>				
			</div>
		</div>
		@else
		...
		<div class="modal-vertical">	
			<div class="modal-content">
					<div class="modal-header">
							...
							{{ trans('contactuslead::template.common.alert') }} 
					</div>
					<div class="modal-body text-center">{{ trans('contactuslead::template.contactleadModule.noExport') }} </div>
					<div class="modal-footer">
							<button type="button" class="btn btn-green-drake" data-dismiss="modal">{{ trans('contactuslead::template.common.ok') }}</button>
					</div>
			</div>
		</div>
	</div>
	...
	<div class="modal-dialog">
		<div class="modal-vertical">	
			<div class="modal-content">
					...
					<div class="modal-body text-center">{{ trans('contactuslead::template.contactleadModule.recordsExport') }}</div>
					<div class="modal-footer">
						<div align="center">
							<div class="md-radio-inline">
								<div class="md-radio">
									...
									<label for="selected_records">
										...
										<span class="box"></span> {{ trans('contactuslead::template.contactleadModule.selectedRecords') }}
									</label>
								</div>
								<div class="md-radio">
									...
									<label for="all_records">
										<span class="inc"></span>
										<span class="check"></span>
										<span class="box"></span>{{ trans('contactuslead::template.contactleadModule.allRecords') }} 
									</label>
								</div>
							</div>
						</div>
						<button type="button" class="btn btn-green-drake" id="ExportRecord" data-dismiss="modal">{{ trans('contactuslead::template.common.ok') }} </button>
					</div>
			</div>
			<!-- /.modal-content -->
		</div>
	...
	<div class="modal-dialog">
		<div class="modal-vertical">	
			<div class="modal-content">
				...
				<div class="modal-body text-center">{{ trans('contactuslead::template.contactleadModule.leastRecord') }} </div>
				<div class="modal-footer">
					<button type="button" class="btn btn-green-drake" data-dismiss="modal">{{ trans('contactuslead::template.common.ok') }} </button>
				</div>
			</div>
...
@section('scripts')
	...
	<script src="{{ url('resources/pages/scripts/packages/contactuslead/contactlead-datatables-ajax.js') }}" type="text/javascript"></script>
