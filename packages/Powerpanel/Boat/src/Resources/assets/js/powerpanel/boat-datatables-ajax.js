...
//				},
		    ...
		    if (tableState) {
		    	  if($.cookie('Boatearch')) {
					      var Boatearch = $.cookie('Boatearch');
	                $('#searchfilter').val(Boatearch);
	                grid.setAjaxParam("searchValue", Boatearch);
	           ...
	           if($.cookie('statusValue') && $.cookie('statusValue') != 'null' && $.cookie('statusValue') != ''){
				...
				grid.init({
						...
						dataTable: { // here you can define a typical datatable settings from http://datatables.net/usage/options 
								...
								"ajax": {
										"url": window.site_url + "/powerpanel/boat/get_list", // ajax source
								},
...
// 	if($.cookie('Boatearch')){
// 		$('#searchfilter').val($.cookie('Boatearch'));
// 		$('#searchfilter').trigger('keyup');	
// 	}
// });
...
function reorder(curOrder, excOrder) {
		var ajaxurl = site_url + '/powerpanel/boat/reorder';
		$.ajax({
				url: ajaxurl,
				data: {
						order: curOrder,
...
function makeFeatured(fid, status) {
		var ajaxurl = site_url + '/powerpanel/boat/makeFeatured';
		$.ajax({
				url: ajaxurl,
