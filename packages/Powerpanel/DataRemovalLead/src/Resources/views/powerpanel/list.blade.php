...
<div class="row">
	<div class="col-md-12">
		...
		<div class="title-dropdown_sec">
				...
				@include('powerpanel.partials.listbreadcrumbs',['ModuleName'=>'Manage Data Removal Leads'])
				@endif
							...
							<div class="row">
								...
								<div class="col-lg-6 col-md-12 col-xs-12" style="display:none;">
									<div class="portlet-rh-title">
										...
										<a class="btn btn-green-drake pull-right btn-rh-search " style="margin-right:15px;" id="contactleadrange" href="javascript:;"><i class="fa fa-search"></i></a>
				...
				<table class="new_table_desing table table-striped table-bordered table-hover table-checkable dataTable" id="datatable_ajax">
					<thead>
						<tr role="row" class="heading">
							...
							<th width="12%" align="left">{{ trans('dataremovallead::template.common.name') }}</th>							
							<th width="15%" align="left">{{ trans('dataremovallead::template.common.emailaddress') }}</th>
							<th width="5%" align="center">Reason for Removal</th>
							<th width="5%" align="center">Request Status</th>
							<th width="5%" align="left">Record Location</th>
							<th width="9%" align="center">{{ trans('dataremovallead::template.common.ip') }}</th>
							<th width="15%" align="center">{{ trans('dataremovallead::template.common.received_date_time') }}</th>
						</tr>
					</thead>
					<tbody></tbody>
				</table>				
				@can('contact-us-delete')
				<a href="javascript:;" class="btn-sm btn btn-outline red right_bottom_btn deleteMass">{{ trans('dataremovallead::template.common.delete') }}</a>
				@endcan
				<a href="#selectedRecords" class="btn-sm btn btn-green-drake right_bottom_btn ExportRecord" data-toggle="modal">{{ trans('dataremovallead::template.dataremovalleadModule.export') }}</a>				
			</div>
		</div>
		@else
		...
		<div class="modal-vertical">	
			<div class="modal-content">
					<div class="modal-header">
							...
							{{ trans('dataremovallead::template.common.alert') }} 
					</div>
					<div class="modal-body text-center">{{ trans('dataremovallead::template.dataremovalleadModule.noExport') }} </div>
					<div class="modal-footer">
							<button type="button" class="btn btn-green-drake" data-dismiss="modal">{{ trans('dataremovallead::template.common.ok') }}</button>
					</div>
			</div>
		</div>
	</div>
	...
	<div class="modal-dialog">
		<div class="modal-vertical">	
			<div class="modal-content">
					...
					<div class="modal-body text-center">{{ trans('dataremovallead::template.dataremovalleadModule.recordsExport') }}</div>
					<div class="modal-footer">
						<div align="center">
							<div class="md-radio-inline">
								<div class="md-radio">
									...
									<label for="selected_records">
										...
										<span class="box"></span> {{ trans('dataremovallead::template.dataremovalleadModule.selectedRecords') }}
									</label>
								</div>
								<div class="md-radio">
									...
									<label for="all_records">
										<span class="inc"></span>
										<span class="check"></span>
										<span class="box"></span>{{ trans('dataremovallead::template.dataremovalleadModule.allRecords') }} 
									</label>
								</div>
							</div>
						</div>
						<button type="button" class="btn btn-green-drake" id="ExportRecord" data-dismiss="modal">{{ trans('dataremovallead::template.common.ok') }} </button>
					</div>
			</div>
			<!-- /.modal-content -->
		</div>
	...
	<div class="modal-dialog">
		<div class="modal-vertical">	
			<div class="modal-content">
				...
				<div class="modal-body text-center">{{ trans('dataremovallead::template.dataremovalleadModule.leastRecord') }} </div>
				<div class="modal-footer">
					<button type="button" class="btn btn-green-drake" data-dismiss="modal">{{ trans('dataremovallead::template.common.ok') }} </button>
				</div>
			</div>
...
@section('scripts')
	<script type="text/javascript">
		window.site_url =  '{!! url("/") !!}';
		var DELETE_URL =  '{!! url("/powerpanel/data-removal-lead/DeleteRecord") !!}';
	</script>
	...
	<script src="{{ url('resources/pages/scripts/packages/dataremovallead/dataremovallead-datatables-ajax.js') }}" type="text/javascript"></script>
