...
use DB;
class DataRemovalLead extends Model {
	/**
	 * The table associated with the model.
	 *
	 * @var string
	 */
	 protected $table = 'data_removal_leads';
	 protected $fillable = [
		'id',
		'fkIntServiceId',
		'varName',
...
/**
		...
		{
				$response      = false;
        $moduleFields = ['id','varName','varEmail','varPhoneNo','txtUserMessage','varReason', 'varRequeststatus','chrDelete','varIpAddress','created_at','updated_at'];       
        $response = Self::getPowerPanelRecords($moduleFields)->deleted()->checkRecordId($id)->first();
        return $response;
		}

		public static function getRecordByEmail($id = false) {
			$response = false;
			if ($id != false) {
				$moduleFields = ['id', 'varName', 'varEmail', 'varReason', 'varRequeststatus'];
	...
	public static function getRecordList($filterArr=false){
		$response = false;
		$moduleFields=[ 'id','fkIntServiceId','varName','varEmail','varPhoneNo','txtUserMessage','varReason', 'varRequeststatus','varIpAddress','created_at','chrPublish'];
		$response = Self::getPowerPanelRecords($moduleFields)
		->deleted()		
		->filter($filterArr)
		->get(); 
	...
	public static function getListForExport($selectedIds=false){
		$response = false;
		$moduleFields=[ 'varName','fkIntServiceId','varEmail','varPhoneNo','txtUserMessage','varReason', 'varRequeststatus','varIpAddress','created_at'];
	...
	function scopeFilter($query, $filterArr = false ,$retunTotalRecords = false) 
	{
				...
				if (!empty($filterArr['servicefilter']) && $filterArr['servicefilter'] != ' ') {
					$data = $query->where('fkIntServiceId', $filterArr['servicefilter']);
				}
