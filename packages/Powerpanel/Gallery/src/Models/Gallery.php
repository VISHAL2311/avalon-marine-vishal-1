...
/**
 * The Gallery class handels bannner queries
 * ORM implemetation.
...
namespace Powerpanel\Gallery\Models;
...
class Gallery extends Model {
    ...
    /**
     * This method handels retrival of gallery records
     * @return  Object
     * @since   2016-07-20
     * @author  NetQuick
     */
    public static function getRecords() {
        $response = false;
        $response = Cache::tags(['Gallery'])->get('getGalleryRecords');
        if (empty($response)) {
            $response = Self::Select(['id', 'varTitle', 'fkIntImgId', 'intDisplayOrder', 'chrPublish'])
                    ->deleted()
                    ->publish()
                    ->paginate(10);
            Cache::tags(['Gallery'])->forever('getGalleryRecords', $response);
        }
        return $response;
    }
     ...
     * @author  NetQuick
     */
    public static function getFrontList($limit) {
        $response = false;
        $moduleFields = [
            'id',
            'varTitle',
        ...
        $response = Cache::tags(['Gallery'])->get('getFrontGalleryList');
        if (empty($response)) {
            $response = Self::getFrontRecords($moduleFields)
                    ...
                    ->orderBy('intDisplayOrder', 'asc')
                    ->paginate($limit);
            Cache::tags(['Gallery'])->forever('getFrontGalleryList', $response);
        }
        return $response;
    }
    
    public static function getFrontHomeListFirst() {
        $response = false;
        $moduleFields = [
            'id',
            'varTitle',
        ...
        $response = Cache::tags(['Gallery'])->get('getFrontHomeListFirst');
        if (empty($response)) {
            $response = Self::getFrontRecords($moduleFields)
                    ->deleted()
                    ->publish()
                    ...
                    ->take(9)
                    ->get();
            Cache::tags(['Gallery'])->forever('getFrontHomeListFirst', $response);
        }
        return $response;
    }
    ...
    public static function getFrontListByalbumId($albumId = false, $page = false, $limit = 10) {
        ...
        $response = Cache::tags(['Gallery'])->get('getFrontGalleryList_' . $page . '_' . $albumId);
        if (empty($response)) {
            $response = Self::select($moduleFields)
                    ->deleted()
                    ->publish()
                    ...
                    ->orderBy('intDisplayOrder', 'asc')
                    ->paginate($limit);
            Cache::tags(['Gallery'])->forever('getFrontGalleryList_' . $page . '_' . $albumId, $response);
        }
        return $response;
    }
    ...
    public static function getChildGrid() {
        $id = $_REQUEST['id'];
        $response = false;
        $moduleFields = ['id', 'varTitle', 'intDisplayOrder', 'chrPublish', 'fkMainRecord', 'created_at', 'chrApproved', 'updated_at', 'intApprovedBy', 'UserID', 'intSearchRank',
            'chrDraft',
            'chrTrash',
            'FavoriteID',
            'created_at',
    ...
    public static function approved_data_Listing($request) {
        ...
        CommonModel::updateRecords($whereConditions, $updateMainRecord, false, 'Powerpanel\Gallery\Models\Gallery');
    ...
    public static function getBuilderGallery($fields, $recIds) {
        $response = false;
        $moduleFields = [
            'id',
            'fkMainRecord',
        ...
        $aliasFields = ['id', 'varAlias'];
        $response = Cache::tags(['Gallery'])->get('getBuilderGallery_' . implode('-', $recIds));
        if (empty($response)) {
            $response = Self::getFrontRecords($moduleFields, $aliasFields)
                    ...
                    ->get();
            Cache::tags(['Gallery'])->forever('getBuilderGallery_' . implode('-', $recIds), $response);
        }
        return $response;
    }

    public static function getAllGallery($fields, $limit, $sdate, $edate) {
        $response = false;
        $moduleFields = [ 
            'id',
            'fkMainRecord',
