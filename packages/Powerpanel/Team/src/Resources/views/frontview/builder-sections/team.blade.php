@if(!empty($data['team']) && count($data['team'])>0)
<section>
    <div class="inner-page-container team-listing">
        <div class="container">
            <div class="row">
                            ...
                            <div class="thumbnail-container">
                                <div class="thumbnail">
                                    <a href="{{ url('team') }}/{{ $team->alias->varAlias }}" title="{{ ucwords($team->varTitle) }}">
                                        <img class="w-100 lazy" data-src="{{ App\Helpers\resize_image::resize($team->fkIntImgId,310,372)}}" src="{{ App\Helpers\resize_image::resize($team->fkIntImgId,310,372)}}" alt="{{ htmlspecialchars_decode($team->varTitle) }}">
                                    </a>
                                </div>
                            </div>	
                        </div>
                ...
                @endforeach
                {{--@if(isset($data['team']) && $data['team']->total() > $data['team']->perPage())
                <div class="col-12">
