...
var Validate = function() {
	var handleBoatCategory = function() {
		$("#frmBoatCategory").validate({
			...
			invalidHandler: function(event, validator) { //display error alert on form submit
				...
				$('.alert-danger', $('#frmBoatCategory')).show();
			},
		...
		$('#frmBoatCategory input').on('keypress',function(e) {
			if (e.which == 13) {
				if ($('#frmBoatCategory').validate().form()) {
					$('#frmBoatCategory').submit(); //form validation success, call ajax form submit
					$("button[type='submit']").attr('disabled','disabled');
				}
				return false;
			}
