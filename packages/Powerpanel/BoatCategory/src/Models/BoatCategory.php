<?php
namespace Powerpanel\BoatCategory\Models;
use Cache;
use Illuminate\Database\Eloquent\Model;
class BoatCategory extends Model
{
    protected $table    = 'boat_category';
    protected $fillable = [
        'id',
        'intAliasId',
        'varTitle',
    ...
    {
        $response = false;
        $response = Cache::tags(['BoatCategory'])->get('getBoatCatRecordIdByAliasID' . $aliasID);
        if (empty($response)) {
            $response = Self::Select('id')->deleted()->publish()->checkAliasId($aliasID)->first();
            Cache::tags(['BoatCategory'])->forever('getBoatCatRecordIdByAliasID' . $aliasID, $response);
        }
        return $response;
    }
    /**
    ...
    {
        $response      = false;
        $boatFields = ['id', 'varTitle'];
        $response      = Cache::tags(['BoatCategory'])->get('getFrontBoatCatList');
        if (empty($response)) {
            $response = Self::getFrontRecords($boatFields)
                ->deleted()
                ->publish()
                ->get()
                ->pluck('varTitle', 'id');
            Cache::tags(['BoatCategory'])->forever('getFrontBoatCatList', $response);
        }
        return $response;
    }
    /**
    ...
    public static function getCategoryList($paginate = 6, $page)
    {
        ...
        $aliasFields  = ['id', 'varAlias'];
        $response     = Cache::tags(['BoatCategory'])->get('getCategoryList_' . $page);
        if (empty($response)) {
            $response = Self::getFrontRecords($moduleFields, $aliasFields)
                ->deleted()
                ->publish()
                ...
                ->paginate($paginate);
            Cache::tags(['BoatCategory'])->forever('getCategoryList_' . $page, $response);
        }
        return $response;
    }
    /**
    ...
    public static function getFrontDetail($id)
    {
        ...
        $aliasFields  = ['id', 'varAlias'];
        $response     = Cache::tags(['BoatCategory'])->get('getBoatCategoryFrontDetail_' . $id);
        if (empty($response)) {
            $response = Self::getFrontRecords($moduleFields, $aliasFields)
                ->deleted()
                ->publish()
                ->checkAliasId($id)
                ->first();
            Cache::tags(['BoatCategory'])->forever('getBoatCategoryFrontDetail_' . $id, $response);
        }
        return $response;
    }
    /**
    ...
    /**
     * This method handels boat-category relation
     * @return  Object
     * @since   2016-07-14
     * @author  NetQuick
     */
    ...
    {
        $response = false;
        $response = $this->hasOne('App\Boat', 'id', 'intCategoryId');
        return $response;
    }
    /**
     * This method handels boat sub-category relation
     * @return  Object
     * @since   2016-07-14
     * @author  NetQuick
     */
    ...
    {
        $response = false;
        $response = $this->hasOne('Powerpanel\BoatCategory\Models\BoatCategory', 'id', 'intParentCategoryId');
        return $response;
    }
    /**
    ...
    {
        $response = false;
        $response = $this->belongsTo('Powerpanel\BoatCategory\Models\BoatCategory', 'intParentCategoryId', 'id');
        return $response;
    }
    /**
