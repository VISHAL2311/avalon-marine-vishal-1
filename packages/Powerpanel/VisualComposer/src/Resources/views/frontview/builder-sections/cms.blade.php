...
<div class="row">
    <div class='col-md-12 col-lg-12 col-xs-12'>
        ...
        <div class="left_img_cms">
            <picture>
                <source type="image/webp" srcset="{!! App\Helpers\LoadWebpImage::resize($data['image']) !!}" data-srcset="{!! App\Helpers\LoadWebpImage::resize($data['image']) !!}">
                <img class="lazy1" data-src="{{ App\Helpers\resize_image::resize($data['image']) }}" src="{{ App\Helpers\resize_image::resize($data['image']) }}" alt="{{ htmlspecialchars_decode($data['title']) }}" title="{{ htmlspecialchars_decode($data['title']) }}">
            </picture>
        </div>
        ...
        <div class="right_img_cms">
            <picture>
                <source type="image/webp" srcset="{!! App\Helpers\LoadWebpImage::resize($data['image']) !!}" data-srcset="{!! App\Helpers\LoadWebpImage::resize($data['image']) !!}">
                <img class="lazy2" data-src="{{ App\Helpers\resize_image::resize($data['image']) }}" src="{{ App\Helpers\resize_image::resize($data['image']) }}" alt="{{ htmlspecialchars_decode($data['title']) }}" title="{{ htmlspecialchars_decode($data['title']) }}">
            </picture>
        </div>
        ...
        <div class="center_img_cms">
            <picture>
                <source type="image/webp" srcset="{!! App\Helpers\LoadWebpImage::resize($data['image']) !!}" data-srcset="{!! App\Helpers\LoadWebpImage::resize($data['image']) !!}">
                <img class="lazy3" data-src="{{ App\Helpers\resize_image::resize($data['image']) }}" src="{{ App\Helpers\resize_image::resize($data['image']) }}" alt="{{ htmlspecialchars_decode($data['title']) }}" title="{{ htmlspecialchars_decode($data['title']) }}">
            </picture>
        </div>
        @endif
        ...
        <div class="download_files clearfix">
            ...
            <ul>
                ...
                if($val->fk_folder != 0){
                    $folderNameobj = App\Document::getFolderName($val->fk_folder);
                    if(!empty($folderNameobj)){
                        $folderName = $folderNameobj->foldername;
                    }
                }
                @endphp
                @if($folderName == "")
                ...
                @else
                <li><a {!! $blank !!} href="{{ $CDN_PATH.'documents/'. $folderName .'/'.$val->txtSrcDocumentName.'.'.$val->varDocumentExtension }}" title="{{ $val->txtDocumentName }}.{{ $val->varDocumentExtension }}"><i class="{{ $icon }}"></i>{{ $val->txtDocumentName }}.{{ $val->varDocumentExtension }}</a></li>
                @endif
                @endforeach
            </ul>
            @endif
        ...
        <div class="left_img_cms">
            <picture class="responsive_commun">
                <source type="image/webp" srcset="{!! App\Helpers\LoadWebpImage::resize($data['image']) !!}" data-srcset="{!! App\Helpers\LoadWebpImage::resize($data['image']) !!}">
                <img class="lazy4" data-src="{{ App\Helpers\resize_image::resize($data['image']) }}" src="{{ App\Helpers\resize_image::resize($data['image']) }}" alt="{{ htmlspecialchars_decode($data['title']) }}" title="{{ htmlspecialchars_decode($data['title']) }}">
            </picture>
        </div>
        @if($data['title'] != '')
        ...
        <div class="right_img_cms">
            <picture class="responsive_commun">
                <source type="image/webp" srcset="{!! App\Helpers\LoadWebpImage::resize($data['image']) !!}" data-srcset="{!! App\Helpers\LoadWebpImage::resize($data['image']) !!}">
                <img class="lazy5" data-src="{{ App\Helpers\resize_image::resize($data['image']) }}" src="{{ App\Helpers\resize_image::resize($data['image']) }}" alt="{{ htmlspecialchars_decode($data['title']) }}" title="{{ htmlspecialchars_decode($data['title']) }}">
            </picture>
        </div>
        @if($data['title'] != '')
        <div class="same_title">
        ...
        <picture class="only_top_img  responsive_commun">
            <source type="image/webp" srcset="{!! App\Helpers\LoadWebpImage::resize($data['image']) !!}" data-srcset="{!! App\Helpers\LoadWebpImage::resize($data['image']) !!}">
            <img class="lazy6 " data-src="{{ App\Helpers\resize_image::resize($data['image']) }}" src="{{ App\Helpers\resize_image::resize($data['image']) }}" alt="{{ htmlspecialchars_decode($data['title']) }}" title="{{ htmlspecialchars_decode($data['title']) }}">
        </picture>
        @if($data['title'] != '')
        <div class="same_title">
            <h2 class="title_div">{{ $data['title'] }}</h2>
        ...
        {!! $data['content'] !!}
        <div class="ac-mb-xs-15"></div>
        <picture class="only__bottom responsive_commun">
            <source type="image/webp" srcset="{!! App\Helpers\LoadWebpImage::resize($data['image']) !!}" data-srcset="{!! App\Helpers\LoadWebpImage::resize($data['image']) !!}">
            <img class="lazy7" data-src="{{ App\Helpers\resize_image::resize($data['image']) }}" src="{{ App\Helpers\resize_image::resize($data['image']) }}" alt="{{ htmlspecialchars_decode($data['title']) }}" title="{{ htmlspecialchars_decode($data['title']) }}">
        </picture>

        @elseif(isset($data['alignment']) && $data['alignment'] == 'center-txt')
        @if($data['title'] != '')
        <div class="ac-mb-xs-15"></div>
        <picture class="only__center responsive_commun">
            <source type="image/webp" srcset="{!! App\Helpers\LoadWebpImage::resize($data['image']) !!}" data-srcset="{!! App\Helpers\LoadWebpImage::resize($data['image']) !!}">
            <img class="lazy" data-src="{{ App\Helpers\resize_image::resize($data['image']) }}" src="{{ App\Helpers\resize_image::resize($data['image']) }}" alt="{{ htmlspecialchars_decode($data['title']) }}" title="{{ htmlspecialchars_decode($data['title']) }}">
        </picture>
        ...
        <div class="cms about-left animated fadeInUp">
            <div class='about_full one_clm_top one_clm_center'>
        ...
        <div class="cms about-left animated fadeInUp">
            <div class='about_full '>
                <div class="about_image one_clm_top">
                    <iframe width="100%" height="315" src="{{ $data['vidId'] }}?rel=0;&autoplay=1" frameborder="0" allow="autoplay;"></iframe>
                </div>
                @if($data['videotitle'] != '')
                <div class="same_title">
        ...
        <div class="location_map">
            <iframe src="https://maps.google.com/maps?q={{ $data['latitude'] }}, {{ $data['longitude'] }}&output=embed&zoom=9" width="100%" height="300" frameborder="0" style="border:0"></iframe>
        </div>
