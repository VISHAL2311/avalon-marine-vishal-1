            ...
            @foreach($data['work'] as $index => $workImages)            
                <div class="item">   
                    <div class="thumbnail-container"><div class="thumbnail">                
                        <img class="lazy" data-src="{!! App\Helpers\resize_image::resize($workImages->fkIntImgId,1349,604) !!}" src="{!! App\Helpers\resize_image::resize($workImages->fkIntImgId,1349,604) !!}" alt="{{$workImages->varTitle}}" title="{{ucwords($workImages->varTitle)}}" />
                    </div></div>
                </div>
            @endforeach
            </div>
