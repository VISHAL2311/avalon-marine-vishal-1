...
class Services extends Model {
     ...
     * @author  NetQuick
     */
    public static function getServiceList($fields, $recIds) {
        $response = false;
        $moduleFields = [
            'id',
            'intAliasId',
    ...
    public static function getTemplateServiceList($fields, $filterArr = false) {
        ...
        if (Request::segment(1) != '') {
            $response = $query->orderBy('intDisplayOrder', 'DESC')->paginate(4);
        } else {
            $response = $query->get();
        }
    ...
    public static function getSidebarRecordList() {
        ...
        $aliasFields = ['id', 'varAlias'];
        $response = Cache::tags(['Services'])->get('getFrontLatestServicesList');
        if (empty($response)) {
            $response = Self::getFrontRecords($moduleFields, $aliasFields)
                    ->deleted()
                    ->publish()
            ...
            Cache::tags(['Services'])->forever('getFrontLatestServicesList', $response);
        }
        return $response;
    }
    ...
    public static function getFrontServicesDropdownList($id = false) {
        $response = false;
        $moduleFields = ['id','varTitle', 'fkIntImgId', 'intAliasId', 'intDisplayOrder', 'created_at'];
        $aliasFields = ['id', 'varAlias'];
        $response = Cache::tags(['Services'])->get('getFrontLatestServicesList_' . $id);
        if (empty($response)) {
            $response = Self::getFrontRecords($moduleFields, $aliasFields)
...
//                    ->latestRecord($id)
    ...
    public static function getServiceNameById($id = false) {
        $response = false;
        $servicesFields = ['varTitle'];
        $response = Self::getPowerPanelRecords($servicesFields)->where('id', $id)->first();
        return $response;
    }
