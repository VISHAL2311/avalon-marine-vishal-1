...
class FrontController extends Controller
{
    ...
    public function __construct()
    {
        ...
        if (!Request::ajax()) {
            ...
            foreach ($menu_content as $element) {
                ...
                $homeActive = '';
                if(Request::segment(1) == null && $element->id == '1'){
                    $homeActive = 'active';
                }else{
                    $homeActive = '';
                }
                $html .= '<li class="first '.(isset($homeActive) && !empty($homeActive) ? $homeActive : '').' ' . $parentactiveclass . '"><a href="' . $menuurl4 . '" title="' . ucfirst($element->varTitle) . '">' . ucfirst($element->varTitle) . '</a>';
            ...
            $FooterMenu = Menu::getHomeFooterFrontRecords();
    ...
    public function shareData()
    {
        ...
        //        if ($url != '') {
        //            if (Request::segment(1) != 'previewpage' && Request::segment(1) != 'search' && Request::segment(1) != 'news-letter' && Request::segment(1) != 'events-calender' && Request::segment(2) != 'preview' && Request::segment(3) != 'preview' && Request::segment(1) != 'sitemap' && Request::segment(1) != 'check_activity') {
        //
        ...
        if (Request::segment(1) == 'site-map') {
            $shareData['META_TITLE'] = 'All the web links of Douglas Construction';
            $shareData['META_KEYWORD'] = '';
            $shareData['META_DESCRIPTION'] = 'All the web links of Douglas Construction';
        } else {
        ...
        if (!empty($objContactInfo)) {
            $shareData['objContactInfo'] = $objContactInfo;
        }
