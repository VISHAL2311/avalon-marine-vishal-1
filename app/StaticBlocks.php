...
class StaticBlocks extends Model
{
		...
		static function get_page_by_slug($slug = false) {
			$response = false;
			if (!empty($slug)) {
				$data = DB::table('cms_page')
				->select(
					'cms_page.id','cms_page.varTitle as Title', 'cms_page.intFKModuleCode', 'cms_page.txtDescription', 'cms_page.chrPublish', 'cms_page.chrDelete', 'cms_page.varMetaTitle', 'cms_page.varMetaKeyword', 'cms_page.varMetaDescription', 'cms_page.created_at', 'cms_page.updated_at', 'alias.varAlias', 'module.id', 'module.varModuleName', 'module.varTableName', 'module.varTitle', 'module.intDisplayOrder'
					)
					->leftJoin('alias', 'cms_page.intAliasId', '=', 'alias.id')
					->leftJoin('module', 'module.id', '=', 'cms_page.intFKModuleCode')
					->where('cms_page.id', '=', $slug)
					->where('cms_page.chrPublish', '=', 'Y')
					->where('cms_page.chrDelete', '=', 'N')->first();
					$response = $data;
				}
				return $response;
			}
