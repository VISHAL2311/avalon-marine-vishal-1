...
class LoadWebpImage {
    ...
    public static function resize($imageID = false, $width = false, $height = false) {
        ...
        if (!empty($images)) {
            ...
            $extension = $images->varConvertedImageExtension;
            if (!empty($width) && !empty($height)) {
                if (!empty($images) && $imageID != 0 && !empty($imageName) && !empty($extension) && $extension != 'svg') {
                    $imagePath = public_path() . '/assets/images/webp/' . $imageName . '.' . $extension;
                    if (file_exists($imagePath)) {
                        ...
                        $response = Config::get('Constant.ENV_APP_URL') . 'assets/images/' . $imageName . '.' . $images->varImageExtension;
                    }
                } else {
                    $bitmapfile = public_path() . '/assets/images/webp/' . $imageName . '.' . $extension;
                    if (realpath($bitmapfile)) {
                        $response = Config::get('Constant.ENV_APP_URL') . 'assets/images/webp/' . $imageName . '.' . $extension;
                    }
                }
            } else {
                if ($imageID != 0 || !empty($imageID)) {
                    if (file_exists(public_path() . '/assets/images/webp/' . $imageName . '.' . $extension)) {
                        $response = Config::get('Constant.ENV_APP_URL') . 'assets/images/webp/' . $imageName . '.' . $extension;
                    }
                }
            }
