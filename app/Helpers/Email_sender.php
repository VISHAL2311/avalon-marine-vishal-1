...
class Email_sender
{
     ...
     * @author  NetQuick
     */
    public static function sendEmail_ViaSMTP($view = null, $settings = null, $logId = null, $replymail = null)
    {
        $reponse = false;
        if (!empty($settings) && $logId > 0 && $view != null) {

            $replyTo = isset($replymail) ? $replymail : Mylibrary::getLaravelDecryptedString(Config::get('Constant.DEFAULT_REPLYTO_EMAIL'));
            if (empty($replyTo)) {
    ...
    public static function sendEmail($view = null, $settings = null, $logId = null, $replymail = null)
    {
        ...
        if (Config::get('Constant.USE_SMTP_SETTING') == 'Y') {
            $returnReponse = Self::sendEmail_ViaSMTP($view, $settings, $logId, $replymail);
        } else {
            if (!empty($settings) && $logId > 0 && $view != null) {
